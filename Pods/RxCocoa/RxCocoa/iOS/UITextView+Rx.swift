...
extension Reactive where Base: UITextView {
    ...
    public var didBeginEditing: ControlEvent<()> {
       ControlEvent<()>(events: self.delegate.methodInvoked(#selector(UITextViewDelegate.textViewDidBeginEditing(_:)))
            .map { _ in
                ()
            })
    }
    ...
    public var didEndEditing: ControlEvent<()> {
        ControlEvent<()>(events: self.delegate.methodInvoked(#selector(UITextViewDelegate.textViewDidEndEditing(_:)))
            .map { _ in
                ()
            })
    }
    ...
    public var didChange: ControlEvent<()> {
        ControlEvent<()>(events: self.delegate.methodInvoked(#selector(UITextViewDelegate.textViewDidChange(_:)))
            .map { _ in
                ()
            })
    }
    ...
    public var didChangeSelection: ControlEvent<()> {
        ControlEvent<()>(events: self.delegate.methodInvoked(#selector(UITextViewDelegate.textViewDidChangeSelection(_:)))
            .map { _ in
                ()
            })
    }
